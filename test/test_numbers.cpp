...
static void test_integer_boundaries() {
  {
    auto r = parse("0");
    CHJSON_CHECK(!r.err);
    CHJSON_CHECK(r.doc.root().is_number());
    CHJSON_CHECK(r.doc.root().is_int());
    CHJSON_CHECK(r.doc.root().as_int() == 0);
  }
  {
    auto r = parse("-0");
    CHJSON_CHECK(!r.err);
    ...
    CHJSON_CHECK(dump(r.doc.root()) == "0");
  }
  {
    auto r = parse("9223372036854775807");
    CHJSON_CHECK(!r.err);
    CHJSON_CHECK(r.doc.root().is_int());
    CHJSON_CHECK(r.doc.root().as_int() == (std::numeric_limits<std::int64_t>::max)());
  }
  {
    auto r = parse("-9223372036854775808");
    CHJSON_CHECK(!r.err);
    CHJSON_CHECK(r.doc.root().is_int());
    CHJSON_CHECK(r.doc.root().as_int() == (std::numeric_limits<std::int64_t>::min)());
  }
  {
    ...
    CHJSON_CHECK(r.doc.root().is_number());
    CHJSON_CHECK(!r.doc.root().is_int());
    CHJSON_CHECK(dump(r.doc.root()) == token);
    (void)r.doc.root().as_double();
  }
}
...
static void test_fraction_and_exponent_tokens_preserved() {
  ...
  for (const char* token : cases) {
    ...
    CHJSON_CHECK(r.doc.root().is_number());
    CHJSON_CHECK(!r.doc.root().is_int());
    CHJSON_CHECK(dump(r.doc.root()) == std::string(token));
    (void)r.doc.root().as_double();
  }
}
